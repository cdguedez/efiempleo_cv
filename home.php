...
$user_admin = wp_get_current_user();
...
$csv = $csv->getCsv(EFI_UPLOADS_TMP.'test-efiempleo.csv');
$arraycsv = $csv['csv'];
$typepost = "resume";
...
for($a=0;$a<count($arraycsv);$a++) {
    for($i=0;$i<count($jsonusermeta);$i++) {
        if($arraycsv[$a][2] == $jsonusermeta[$i]->user_email) {
            ...
            $candidatename = ucwords(strtolower($jsonusermeta[$i]->display_name));
...
echo '<br>'. round(memory_get_usage()/1048576) . "MB";